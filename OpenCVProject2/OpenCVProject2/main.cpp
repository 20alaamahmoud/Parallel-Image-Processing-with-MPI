...
//Final Project1 -> 6 Filter
enum Operation { COLOR_SPACE_CONVERSION, GLOBAL_THRESHOLDING, LOCAL_THRESHOLDING, EDGE_DETECTION, IMAGE_ROTATION, HISTOGRAM_EQUALIZATION };
...
int main(int argc, char** argv)
{
    ...
    int thresholdValue = 0; // Initialize threshold value variable
    ...
    {
        std::cout << std::endl;
        std::cout << "--- Welcom to Parallel Image Processing With MPI ---" << std::endl;
        std::cout << std::endl;
        ...
        std::cout << "4: Image Rotation" << std::endl;
        ...
        if (operation == COLOR_SPACE_CONVERSION)
            ...
            selectedOperation = HISTOGRAM_EQUALIZATION;
        else
        {
    ...
    else if (selectedOperation == GLOBAL_THRESHOLDING)
    {
        ...
        cv::cvtColor(localImage, resultImage, cv::COLOR_BGR2GRAY);
        cv::threshold(resultImage, resultImage, thresholdValue, 255, cv::THRESH_BINARY);
    }
    ...
    else if (selectedOperation == HISTOGRAM_EQUALIZATION)
    {
        if (rank == 0)
        {
            std::cout << "Enter the name of the output image: ";
    ...
    if (rank == 0)
    {
        ...
        std::cout << std::endl;
        std::cout << std::endl;
        std::cout << "---<<< From the River to the Sea Palestine Will be Free >>>---" << std::endl;
    }
...
//Final Project2 -> 8 Filter
/*
#include <iostream>
#include "mpi.h"
#include "opencv2/opencv.hpp"

enum Operation { COLOR_SPACE_CONVERSION, GLOBAL_THRESHOLDING, LOCAL_THRESHOLDING, MEDIAN_FILTER, EDGE_DETECTION, GAUSSIAN_BLUR, IMAGE_ROTATION, IMAGE_SCALING, HISTOGRAM_EQUALIZATION };
...
int main(int argc, char** argv)
{
    ...
    if (rank == 0)
    {
        ...
        std::cout << "8: Histogram Equalization" << std::endl;
